...
const EditJob = () => {
  ...
  const initiateupdatedata = async () => {
    ...
    setJobRole(response.data.jobRoles);
    setJobNature(response.data.jobNature);
    setTotalVacancies(response.data.totalVacancies);
    setJobDescription(response.data.jobDescription);
    setRequiredSkills(response.data.requiredSkills);
    setApplicationStartDate(response.data.applicationStartDate.split("T")[0]);
    setApplicationEndDate(response.data.applicationEndDate.split("T")[0]);
    setSalaryPackage(response.data.salaryPackage);
    setExperienceRequired(response.data.experienceRequired);
    setDegreesPreferred(response.data.degreesPreferred);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put("http://localhost:5000/api/job/"+id, {
        ...jobDetails,
      });
      console.log("Job details updatedted successfully:", response.data);
      alert("Job details posted successfully!");
    } catch (error) {
      console.error("Error posting job details:", error);
      alert("Error update job details. Please try again.");
    }
  };
    ...
    <div className="flex w-full">
      ...
      <div className="bg-gray-100 flex justify-center items-center w-full ">
        ...
        <div className="w-[90%] sm:w-[70%] lg:w-[100%] h-[80%] sm:h-[60%] lg:h-[100%] bg-white p-8 rounded-lg shadow-lg">
          <h2 className="text-3xl font-bold text-center mb-4 bg-blue-500 p-6 rounded-lg">
            Post Your Job
          </h2>
